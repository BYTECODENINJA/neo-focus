...
import { Plus, Search, Edit, Trash2, Tag, Save, X, FileText, FolderOpen, History } from "lucide-react"
...
export function Notebook({ notes, setNotes }: NotebookProps) {
  ...
  // Add keyboard shortcut for creating new notes
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Ctrl+S or Cmd+S to save note when editing
  ...
  const filteredNotes = notes.filter((note) => {
    ...
    // If searching by tag, ignore the folder filter so tag matches always show
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Folder Display */}
                      {note.category && (
                        <p className="text-xs text-purple-700 dark:text-purple-300 mt-1">{note.category}</p>
                      )}
                ...
                <div className="mb-4">
                  ...
                  <div className="flex gap-2 flex-wrap items-center">
                    <Select
                      ...
                      onValueChange={(v) => {
                        if (v === "new") {
                          // Focus the new folder input when "Create New" is selected
                          setTimeout(() => {
                  ...
                  {editingNote.category && (
                    <div className="mt-2 flex items-center gap-2">
                      <span className="text-xs px-2 py-1 rounded-full bg-purple-500/20 text-purple-800 dark:text-purple-300">
                        {editingNote.category}
                      </span>
                      <button
                        onClick={() => setEditingNote({ ...editingNote, category: "" })}
